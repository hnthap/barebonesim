...
Mỗi dấu cách đều có thể thay bằng ký tự xuống dòng hay *tab* (bốn space) để dễ đọc, chẳng hạn `while X not 0 do decr X end;` thì cũng tương đương với
...
Khuyến khích viết thành các dòng để code để đọc hơn.
...
Có thể biểu diễn số âm trong BB bằng cách dùng một biến dấu đi chung với số không âm. Biến dấu mang giá trị $0$ nếu $X \ge 0$ và mang giá trị khác $0$ trong trường hợp ngược lại.
...
![](C:\Users\C15TQK\AppData\Roaming\marktext\images\2023-05-11-15-56-54-image.png)
...
> Ngoài ra, các biến này có thể được thêm từ file CSV. Nhấn nút ` From CSV ` và chọn file cần tìm. File CSV tuân theo [RFC 4180](https://www.ietf.org/rfc/rfc4180.txt) và *không chứa dòng header*.
> 
...
4. Trên cửa sổ "Snippet Settings", thay đổi tên các biến như bên dưới rồi chọn OK.
   ...
   ![](D:\Code\personal\barebonesim\Screenshot_03.png)
   
   > Chú ý đặt tên các biến tạm thời (temporary variables) sao cho không trùng với biến nào đang dùng. Các biến tạm thời trong một phép toán không được trùng tên. Tên biến tạm thời nên bắt đầu với "T" hoặc "T_".
   > 
   > Chúng có thể được tái sử dụng nhiều lần. Sau mỗi phép tính cung cấp bởi snippet, các biến tạm đều mang giá trị không.

5. Trong cửa sổ mới hiện ra, tô chọn tất cả đoạn code trong khung và nhấn tổ hợp Ctrl + C để sao chép, rồi chọn OK.
   
   ![](D:\Code\personal\barebonesim\Screenshot_04.png)
...
7. Bấm nút `[+]` trên cửa sổ Input (bên trái cửa sổ soạn thảo) rồi điền tên và giá trị ban đầu của biến `Z` (ví dụ, bằng $12$).
   ...
   ![](D:\Code\personal\barebonesim\Screenshot_06.png)
   
   Làm tương tự với biến `T` (ví dụ, bằng $15$).
   
   ![](D:\Code\personal\barebonesim\Screenshot_07.png)
   
   Cửa sổ Input sẽ hiển thị như sau :
   
   ![](D:\Code\personal\barebonesim\Screenshot_08.png)

8. Bấm nút `▶` trên thanh công cụ để thực thi đoạn code. Có thể thấy kết quả gần giống như hình dưới :
   
   ![](D:\Code\personal\barebonesim\Screenshot_09.png)
